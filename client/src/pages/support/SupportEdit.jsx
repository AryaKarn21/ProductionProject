...
export default function SupportEdit() {
  ...
  const { data: usersData } = useQuery({
    queryKey: ['users'],
    queryFn: () => settingsAPI.getUsers({ page: 1, limit: 1000 }).then(res => res.data),
  })
  ...
  return (
    <div className="animate-fade-in">
      ...
      <ConfirmDialog
        open={showConfirm}
        onConfirm={() => { setShowConfirm(false); navigate(`/support/${id}`) }}
      ...
      <div className="page-header">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[18px] font-bold">Edit Support Ticket</h1>
            <p className="text-[12px] text-gray-500">Update ticket details</p>
          </div>
        </div>
      </div>
